          ...
          this.ocrService.processPages(
            ...
            async (results: PageOcrResult[]) => {
              try {
                ...
                onProgress({ stage: 'creating', percent: 50, status: 'Đang tạo PDF có thể tìm kiếm' });
                
                // Tạo PDF chỉ chứa text
                onProgress({ stage: 'creating', percent: 51, status: 'Đang tạo PDF chỉ chứa text' });
                ...
                const searchablePdfBlob = new Blob([searchablePdfBytes], { type: 'application/pdf' });
